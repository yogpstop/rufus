...
static transformer_state_t *setup_transformer_on_fd(int fd, int fail_if_not_compressed)
{
	...
	if (ENABLE_FEATURE_SEAMLESS_ZST
	 && magic.b16[0] == ZSTD_MAGIC1
	) {
		offset = -4;
		xread(fd, magic.b16 + 1, sizeof(magic.b16[1]));
		if (magic.b16[1] == ZSTD_MAGIC2) {
			xstate->xformer = unpack_zst_stream;
